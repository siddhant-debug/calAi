...
When you finish, report: the new ADR's file path, its number and companion ADRs, and a one-paragraph handoff brief addressed to whichever engineer implements it next — this becomes the "Upstream output" the dispatcher/orchestrator forwards into that engineer's brief.
