...
## Using the `engineering:testing-strategy` skill (available via `Skill`)
Useful for scaffolding an approach on a genuinely new kind of surface (e.g. the first test for a
new subsystem) — general coverage-design thinking. It doesn't know ADR-004's golden-dataset
format, the calorie-range-not-exact-value convention, or the ~15-example noise threshold below —
those rules are this project's own and always win on conflict.
